...
const mockTestSets = [
  { id: "identity-fidelity", name: "Capturing Likeness & Appeal", samples: 300 },
  { id: "form-language", name: "Unified Art Style", samples: 250 },
  { id: "material-expression", name: "Material Expression", samples: 200 },
  { id: "technical-integrity", name: "Technical Quality", samples: 180 },
]
...
export default function ModelEvaluation() {
  ...
  const configureModel = async () => {
    if (modelConfig.name && modelConfig.endpoint) {
      ...
      try {
        // Simulate API call delay for testing connection
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <TabsContent value="execution" className="space-y-6">
              <Alert>
                ...
                <AlertDescription>
                  Configure your model API endpoint and authentication, then execute evaluation tests against your model.
                </AlertDescription>
              </Alert>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex justify-end">
                      <Button
                        ...
                        disabled={!modelConfig.name || !modelConfig.endpoint || isTestingConnection}
                        className={`gap-2 text-white shadow-lg hover:shadow-xl transition-all duration-300 ${
                          connectionStatus === 'success' 
                            ? 'bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 animate-pulse' 
              ...
              {!isConfigured ? (
                <Alert className="border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50">
                  ...
                  <AlertDescription className="text-amber-800">
                    Please configure your model first before running tests.
                  </AlertDescription>
                </Alert>
              ) : (
                <Alert className="border-green-200 bg-gradient-to-r from-green-50 to-emerald-50">
                  ...
                  <AlertDescription className="text-green-800">
                    Model "{modelConfig.name}" is configured and ready for testing.
                  </AlertDescription>
                </Alert>
              )}
                    ...
                    {mockTestSets.map((testSet) => (
                      <div
                        ...
                        className={`flex items-center justify-between p-4 border border-border rounded-lg ${
                          !isConfigured ? 'opacity-50 bg-slate-50' : 'hover:bg-slate-50'
                        }`}
                      >
                        <div>
                        ...
                        <Button 
                          onClick={() => startTest(testSet.id)} 
                          size="sm" 
                          className="gap-2"
                          disabled={!isConfigured}
                        >
                          <Play className="h-4 w-4" />
                          {!isConfigured ? 'Locked' : 'Run Eval'}
                        </Button>
                      </div>
                    ))}
                  </CardContent>
